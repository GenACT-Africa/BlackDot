...
export default function NewTalentPage() {
  ...
  // Temporary ID used as the storage path before the talent row is created
  ...
  async function handleAvatarChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const path = `talents/${tempId}/avatar.${ext}`
  ...
  const initials = nameValue
    ? nameValue.split(' ').map((n: string) => n[0]).join('').toUpperCase().slice(0, 2)
    : '?'
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-black text-white mb-8">Add New Talent</h1>
